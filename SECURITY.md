...
[Report a vulnerability](https://github.com/chrisfulcher/orrery/security/advisories/new).
...
orrery is pre-release. `main` is the only supported version, and a fix lands
there. Once releases begin, this section will name the versions that receive
them.

## Where the security surface is

orrery is self-hosted and runs on the user's own machine against their own
...
- **Credentials.** The user's SAM.gov API key and any model credentials live
  only in their configuration. `.env` is written by `src/orrery/dotenv.py`
  alone — `ORRERY_*` keys only, atomic replace, mode 0600 — and credentials are
...
- **What leaves the machine for inference.** Embedding and semantic search send
  text only to `ORRERY_EMBED_BASE_URL`; chat models are reached only through
  `src/orrery/ai/` at the endpoints configured under `ORRERY_AI_*`. The MCP
...
- **Untrusted input.** Notice text, attachment files, and bulk extracts come
  ...
  are bugs; path traversal out of `ORRERY_DATA_DIR`, SQL injection, terminal
...
- The consequences of a user pointing `ORRERY_EMBED_BASE_URL` or `ORRERY_AI_*`
  at a service they do not trust. Choosing the endpoint is the user's, by
  design; the guarantee is that orrery sends there and nowhere else.
