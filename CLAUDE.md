...
orrery is a free, open source, self-hosted business development intelligence tool for U.S. federal government contracting. Read `docs/DESIGN.md` before proposing anything; it is the source of truth and this file is its condensation. When they disagree, the design doc wins and this file gets fixed.
...
- `notices.description` is plain text from every source. Attachment paths are stored relative to `ORRERY_DATA_DIR`. Fetch failures are recorded with why they failed (a `failure_kind` classified where the error is raised, plus the key-redacted detail), never retried automatically; a stage that succeeds clears the reason it recorded.
- `embeddings` rows record the model that produced them and are rebuilt by deleting a model's rows and re-running `orrery embed`. Embedding and semantic search send text only to `ORRERY_EMBED_BASE_URL`; notice and attachment text is public, the user's query text is not, and the README says so.
- Bulk-extract rows never overwrite an API-sourced notice; they confirm it and fill an unfetched description. `notices.source_id` is the source that last wrote the typed columns. `active` is cleared only by a complete pass of the active extract, only within the configured NAICS slice, and only against the extract's own cut (`ingestion_runs.source_generated_at`) rather than the run's start; a pass that does not know the cut clears nothing.
...
- Exclusions come from the daily public extract, never a keyed API. Rows classified `Individual` are counted and not read past that, and the six person columns are blanked in every stored record. Only a row whose UEI or CAGE exactly matches a contractor already in the store is read, and nothing in the file creates an entity or an alias. An exclusion is `sam.exclusion.*` facts keyed by SAM Number; the file lists active records only, so absence from a later complete dated file becomes a `terminated` status fact with extraction method `absence`, never a deletion, and a capped, cancelled, or undated run ends nothing. "Currently excluded" is derived at read time.
- Offices resolve against the SAM.gov Federal Hierarchy by their FPDS office code (the AAC that ends the agency path), one keyed request per office against the daily budget, never creating an entity. The organization id and that code are columns because both are identity; what the hierarchy says about the office is `fh.*` facts. The canonical name replaces the name a notice happened to carry and the displaced name and every former name become resolved aliases. One organization id belongs to one row: a second office resolving to it gets an `fh.same_as` ref fact pointing at the holder, never a deletion and never a merged-away row. Several organizations claiming one code resolve nothing and are recorded as `fh.candidate` facts; none records `fh.lookup = 'not found'`, which stands for 30 days so the job does not spend the day's requests re-asking.
...
- Source layout `src/orrery/`, tests in `tests/`, CLI entry point `orrery` (typer).
- Interfaces: terminal UI with Textual (`orrery top`, `src/orrery/tui/`), CLI with `--json` on every command that prints data, MCP server (`src/orrery/mcp_server.py`, stdio, interface version 1, tools only added, nothing that spends quota), and versioned read-only SQL views named `v_*` (interface version 1, columns only added). All of them go through one query module (`src/orrery/query.py`); no front end reads the raw tables directly, and the raw tables are not a public interface.
- Long-running operations (ingest, fetch, extract, embed, summarize, assess, db) are jobs in `src/orrery/jobs.py`; the CLI and the app both run them through it, and adapters take `report` and `cancelled` hooks from `src/orrery/progress.py`. The app runs a job on its own connection in a thread; its own connection never leaves the event loop.
- Storage is stdlib `sqlite3`, no ORM. Schema changes are numbered SQL files in `src/orrery/migrations/`, applied in order by `orrery db migrate`, and mirrored in `docs/DESIGN.md` §8 in the same commit.
- Commands: `uv sync` (install), `uv run orrery` (CLI), `uv run pytest` (tests), `uv run ruff check . && uv run ruff format --check .` (lint and format). All four must pass before a commit.
- Container: `docker compose build` and `docker compose run --rm orrery <command>`. The Dockerfile installs only from the lockfile (`uv sync --frozen`), never pulls an application image from a registry, and runs as a non-root user with the store at `/data`.

## Repo hygiene

- No secrets. `.env` files are gitignored; keep them that way.
- `.env` is written only by `src/orrery/dotenv.py`: only `ORRERY_*` keys, comments and order preserved, atomic replace (in place on a bind mount), mode 0600, never logged. Real environment variables still override it and the app shows those fields as locked.
