...
from orrery.documents import DocumentError, ProfileDocument, SearchDocument, WorkflowDocument
...
def version() -> None:
    ...
    typer.echo(f"orrery {__version__}")
...
def extract(
    ...
    """Extract text from fetched attachments (PDF, Word, Excel). Spends no quota."""
...
def top(
    ...
    if serve:
        try:
            ...
            raise typer.Exit(2) from exc
        Server("orrery top", port=port).serve()
        return
    from orrery.tui.app import OrreryTop

    OrreryTop(settings, env_path=Path(".env")).run()


@app.command()
def mcp() -> None:
    """Serve the store to an MCP client over standard input and output."""
    from orrery.mcp_server import main  # heavy import; only when asked for
