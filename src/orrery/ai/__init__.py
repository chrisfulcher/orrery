...
def resolve_slot(settings: Settings, name: SlotName) -> Slot:
    ...
    if model is None:
        if provider != "anthropic":
            raise AIError("ORRERY_AI_DEEP_MODEL is required when ORRERY_AI_DEEP_PROVIDER is openai")
...
def backend_for(slot: Slot) -> ChatBackend:
    ...
    if slot.provider == "anthropic":
        try:
            from orrery.ai.anthropic_backend import AnthropicBackend
        except ImportError as exc:
            raise AIError(
                "the anthropic provider needs the SDK: install the extra with"
                " `uv sync --extra anthropic` (or `pip install 'orrery[anthropic]'`)"
            ) from exc
