...
Every setting is read from the environment as ``ORRERY_<FIELD>`` (for example
``ORRERY_DATA_DIR``), or from a ``.env`` file in the working directory. Real
...
from orrery.naics import validate_slice


class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        env_prefix="ORRERY_", env_file=".env", env_ignore_empty=True, extra="ignore"
    )
    ...
    """The NAICS slice to ingest, comma-separated in ORRERY_NAICS. Each element is a prefix of
    ...
    def db_path(self) -> Path:
        return self.data_dir / "orrery.sqlite"


def env_key(field: str) -> str:
    """``sam_api_key`` -> ``ORRERY_SAM_API_KEY``."""
    return "ORRERY_" + field.upper()
...
def environment_overrides() -> set[str]:
    """Fields whose ``ORRERY_*`` variable is set and non-empty in the process environment:
