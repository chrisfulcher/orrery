...
``ORRERY_NAICS`` holds NAICS *prefixes*, not complete codes. USAspending's award API filters
the same way -- ``5415`` selects every code beneath that industry group, and a longer prefix
wins over a shorter one -- so orrery matches prefixes everywhere it filters by NAICS. The
...
from orrery.progress import Report
...
def matches(code: str | None, prefixes: Iterable[str]) -> bool:
    ...
    slice matches nothing, as an empty ``ORRERY_NAICS`` means nothing to ingest."""
    if not code:
        return False
