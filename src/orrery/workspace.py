...
from orrery.documents import ProfileDocument, SearchDocument, WorkflowDocument
    ...
    if stage is not None:
        ...
        if outcome is not None:
            ...
            if keys.index(key) < keys.index(p.stage):
                raise ValueError(f"{key} is earlier than {p.stage}; use `orrery pursuit back`")
            while p.stage != key:
...
def describe_search(saved: SavedSearch) -> str:
    """The one-line summary ``orrery searches list`` prints."""
