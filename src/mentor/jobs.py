...
settings with progress and cancellation callbacks (``mentor.progress``). ``run`` validates
...
from mentor.progress import Cancelled, JobCancelled, Report, check, never, quiet
...
def needs_unmet(job: Job, settings: Settings, params: dict) -> list[str]:
    ...
    if "sam_key" in job.needs and settings.sam_api_key is None:
        problems.append("MENTOR_SAM_API_KEY is not set")
    if "naics" in job.needs and not settings.naics:
        problems.append("MENTOR_NAICS is empty; nothing to ingest")
...
def _entities_unmet(settings: Settings, params: dict) -> list[str]:
    ...
    return ["MENTOR_SAM_API_KEY is not set"] if needs_key and settings.sam_api_key is None else []
