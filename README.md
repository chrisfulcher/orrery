...
- Budget context
- Names for every office. `orrery ingest hierarchy` resolves them against SAM.gov's
  Federal Hierarchy a few keyed requests at a time, so a fresh store's offices are still
  the codes and names the notices carried until it has run for a while.
...
uv run orrery ingest hierarchy    # optional, needs a key: name your offices (one request each)
...
docker compose run --rm orrery ingest hierarchy
...
| `orrery ingest hierarchy` | Resolves your offices against SAM.gov's Federal Hierarchy by the FPDS office code their agency path ends with, taking the canonical name, the organization id, and the names each office used to go by. One keyed request per office, `--budget N` per run (default 5), so the offices resolve over a few runs; an office the hierarchy does not know is not asked about again for a month. | One request per office |
